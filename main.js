...
global.authFile = 'RoxySession'
global.authFileJB  = 'RoxyJadiBot'
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['RoxyBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['RoxyBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
const {connection, lastDisconnect, isNewLglobal.conn = makeWASocket(connectionOptions)
...
if (opts['server']) (await import('./server.js')).default(global.conn, PORT);ogin} = update
...
const subBotDirectory = './RoxyJadiBot';
if (!existsSync(subBotDirectory)) {
console.log('🤍 Roxy Bot MD no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'RoxySession';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🤍 Roxy Bot MD conectado correctamente.`))
...
let prekey = []
let directorio = readdirSync("./RoxySession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./RoxySession/${files}`)
})
} 
...
const directories = ['./RoxySession/', './RoxyJadiBot/']
